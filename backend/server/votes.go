...
func (vm *VoteManager) StartVotingWithChoices(questionID string, choiceIDs []string, choiceObjects []parser.Choice, duration time.Duration, onComplete func(map[string]int, string)) {
	vm.mu.Lock()
	defer vm.mu.Unlock()
	...
	// Initialize vote counts - always reset to 0 for a fresh vote
	...
	// Broadcast voting started with full choice objects if provided
...
func (vm *VoteManager) SubmitVote(voterID, choiceID string) error {
	...
	if !vm.votingActive {
		return nil // Ignore votes when not active
	}
...
func (vm *VoteManager) ResetVoting() {
	...
	vm.currentQuestion = ""
	vm.voters = make(map[string]string)
	vm.votes = make(map[string]map[string]int) // Clear all vote history
...
func (vm *VoteManager) ClearQuestionVotes(questionID string) {
	...
	// Only clear the specific question's votes
	if questionID != "" {
